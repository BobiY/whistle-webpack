...
class CopyDir {
        ...
        paths.forEach(p => {
            ...
            fs.stat(_src, (err,stats) => {  //stats  该对象 包含文件属性
                ...
                if(stats.isFile()){ //如果是个文件则拷贝
                    ...
                    // copy 文件打印提示信息
                    console.log(`${colors.green('Copy: ')} ${path.join(dirPre || "",path.basename(_src))}`); 
                    let readable=fs.createReadStream(_src);//创建读取流
                    let writable=fs.createWriteStream(_dst);//创建写入流
                    readable.pipe(writable);
                }else if(stats.isDirectory()){ //是目录则 递归 
                    console.log(`${colors.green('Create: ')} ${path.basename(_src)}`);
                    this.checkDirectory(_src,_dst,path.basename(_src));
                }
            });
        });
    }
     ...
     */
    checkDirectory(src, dst, dirPre) {
        fs.access(dst, fs.constants.F_OK, (err) => {
            if(err){
                fs.mkdirSync(dst);
                // console.log(`${colors.green('Copy: ')} ${dst}`);
            }
            this.copy(src,dst,dirPre);
        });
    }
}
